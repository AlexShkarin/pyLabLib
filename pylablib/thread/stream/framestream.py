...
class FrameSaveThread(controller.QTaskThread):
    ...
    Receives frame multicasts, and saves the frames to the disk.

    Setup args:
        - ``src``: frames source
        - ``tag``: frames multicast tag
            ...
            if new_chunk:
                ...
                if self._perform_status_check:
                    if self.v["status_line_check"] in {"ok","na"}:
                        self.v["status_line_check"]=self._check_status_line(flat_chunk,step=new_chunk[0].step)
    ...
    def write_settings(self, extra_settings=None):
        ...
        savefile.save_dict(settings,self._get_settings_path(self.v["path"]))
    def finalize_settings(self):
        """Save finalized settings to the file"""
        path=self._get_settings_path(self.v["path"])
        if os.path.exists(path):
    ...
    def _get_snapshot_background_parameters(self):
        if self.frame_processor:
            try:
                ...
                return frame_processor.get_background_to_save(),frame_processor.v["snapshot_mode"]
            except controller.threadprop.NoControllerThreadError:
                pass
        return None,None
    def write_background(self):
        ...
        if background is not None:
            ...
            with open(self._get_background_path(self.v["path"]),"wb") as f:
    ...
    def write_event_log(self, msg):
        ...
        if self._saving:
            path=self._get_event_log_path(self.v["path"])
            preamble=""
        ...
        if self._saving and not self._stopping:
            ...
            if tot_frames:
                ...
                if self.v["queue_ram"]<=self.v["max_queue_ram"]:
                    ...
                    self.v["missed"]+=msg.get_missed_frames(self._last_frame_idx if msg.first_frame_index() else None) # don't count reset as skip
    ...
    def receive_frames(self, src, tag, msg):
        """Process frame receive multicast"""
        scheduled=self.schedule_message(msg)
